...
const web3 = new Web3(new Web3.providers.HttpProvider("https://ropsten.infura.io/v3/2413e0137d6b4a3181d29c8f7727fcf6"));
...
const Salon = {
  ...
  init: async function () {
    ...
    // if (abi === process.env.VUE_APP_SALONABI) {
    //   alert("abi===VUE_APP_SALONABI")
    // }
  ...
  newCampaign: async function (campaignID, topic, speaker, sponsor) {
    let self = this;
    const data = self.instance.methods.newCampaign(campaignID, topic, speaker, sponsor).encodeABI();
    const transaction = {
    ...
    const res = await tp.sendEthTransaction(transaction);
    alert(res.data)
    return res.result;
